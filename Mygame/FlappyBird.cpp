	...
	for (int i = 0; i < gameTime; i++) {
		...
		if (counter == 5) {
			//for (int i = 1; i < FLAPPYBIRDOBJECTS; i++)
				//object[i]->setScaling(D3DXVECTOR2(randomNumber,randomNumber));
		}
	}
...
void FlappyBird::draw()
{
	...
	graphics->drawfont("Score = ", timer * 10, 11, 500, 50, cursor->sprite, D3DCOLOR_XRGB(254, 0, 0), 30);			// last parameter depends on the size of your font
	cursor->draw();
	object[0]->sprite->End();
...
void FlappyBird::collisions()
{
	...
	if (object[1]->jumpCD >= 60 && input->upArrowKey) {
		...
		for (int i = 0; i < FLAPPYBIRDOBJECTS; i++) {
			if (i == 1)i++;
			object[i]->setVelocity(D3DXVECTOR2(object[i]->getSpeed(), (1.5*sin(object[i]->getObjectX()))));
